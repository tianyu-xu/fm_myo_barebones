...
class MazeGame(object):
    ...
    def move(self, mv):
        ...
        # 判断大于等于30s为超时
        print(round(self.end - self.start,0))
    ...
    def location(self, tx, ty):
        ...
        if (((xlen == 1) and (ylen == 0)) or ((xlen == 0) and (ylen == 1))) and (
                ((x1 > tx) and (up)) or ((x1 < tx) and (down)) or ((y1 > ty) and (left)) or ((y1 < ty) and (right))):
            self.walker = (tx, ty)
            self.disp.setGoal1(tx, ty)
        else:
            # messagebox.showwarning("小提示", "warning!!!")
            print("waring!!!")
        if(tx==xx and ty==yy):
            return "no"
            # messagebox.showwarning("小提示", "you win!!!")
    ...
    # 自动走迷宫
    def auto2(self, x, y, tx, ty):
        r, c = self.walker  # 从哪里开始的
        xx, yy = (tx, ty)
        print('起点坐标 x、y:', r, c, '终点坐标x、y:', xx, yy)
        # exit()
        self.answer2(r, c, x, y, tx, ty)

    # 自动走迷宫得到答案的具体递归代码
    # 递归的“核心”代码
    def answer2(self, i, j, x, y, tx, ty):
        x1, y1 = self.walker  # 当前点所在位置 (0,7)
        xx, yy = (tx, ty)  # 结束的出口前面(9,3)
        found = None
        ...
        if (not found) and (i - 1 >= 0) and (up) and ((i - 1, j) not in self.visited):
            ...
            self.walker = (i - 1, j)  # 格子代码走了一步
            self.disp.moveWalkerAnswer(i - 1, j)  # 格子在界面上的显示走了一步
            self.answer2(i - 1, j, x, y, tx, ty)  # 又递归调用一次代码，直到走出去的时候标志位为True时结束

        if (not found) and (i + 1 <= x + 1) and (down) and ((i + 1, j) not in self.visited):
            print('往下走')
            self.walker = (i + 1, j)
            self.disp.moveWalkerAnswer(i + 1, j)
            self.answer2(i + 1, j, x, y, tx, ty)

        if (not found) and (j - 1 >= 0) and (left) and ((i, j - 1) not in self.visited):
            print('往左走')
            self.walker = (i, j - 1)
            self.disp.moveWalkerAnswer(i, j - 1)
            self.answer2(i, j - 1, x, y, tx, ty)
