...
def main():
    ...
    freq = 10
    speed = 15  # a content speed
    #move = cursor(freq,speed)
    ...
    with hub.run_in_background(emg_buffer.on_event):
        ...
        while hub.running:
            ...
            # ...
            #move.move_cursor(inference[0])
            if gestures[inference[0]] == 'fist':
                pyautogui.move(0,0,duration=1/freq)
            if gestures[inference[0]] == 'radial':
                pyautogui.move(-speed, 0,duration=1/freq)
            if gestures[inference[0]] == 'ulnar':
                pyautogui.move(speed, 0,duration=1/freq)
            if gestures[inference[0]] == 'extension':
                pyautogui.move(0 , -speed,duration=1/freq)
            if gestures[inference[0]] == 'flextion':
                pyautogui.move(0 , speed,duration=1/freq)
            if gestures[inference[0]] == 'rest':
                pyautogui.move(0 , 0,duration=1/freq)
